 ...
 */
#define WINBOND_COMMAND_RANDOM_READ	0x03         /* Random read command */
#define WINBOND_COMMAND_PAGEPROGRAM_WRITE	0x02 /* Page Program command */
#define	WINBOND_COMMAND_WRITE_ENABLE	0x06     /* Write Enable command */
#define WINBOND_COMMAND_SECTOR_ERASE	0xD8     /* Block Erase command */
#define WINBOND_COMMAND_BULK_ERASE	0xC7         /* chip Erase command */
#define WINBOND_COMMAND_STATUSREG_READ	0x05     /* Status read command */
#define WINBOND_COMMAND_STATUSREG_WRITE	0x01     /* Status write command */
...
#define WINBOND_BULK_ERASE_BYTES		1 /* Bulk erase extra bytes */
 ...
 * the flash device.
 */
#define WINBOND_DISABLE_PROTECTION_ALL	0x00
...
#define WINBOND_DUMMYBYTE		0xFF /* Dummy byte */
...
int SpiWinbondFlashWriteEnable(XSpi *SpiPtr);
int SpiWinbondFlashWrite(XSpi *SpiPtr, u32 Addr, u32 ByteCount);
int SpiWinbondFlashRead(XSpi *SpiPtr, u32 Addr, u32 ByteCount);
int SpiWinbondFlashBulkErase(XSpi *SpiPtr);
int SpiWinbondFlashSectorErase(XSpi *SpiPtr, u32 Addr);
int SpiWinbondFlashGetStatus(XSpi *SpiPtr);
int SpiWinbondFlashWriteStatus(XSpi *SpiPtr, u8 StatusRegister);
static int SpiWinbondFlashWaitForFlashNotBusy(void);
 ...
 */
u8 ReadBuffer[WINBOND_FLASH_PAGE_SIZE + WINBOND_READ_WRITE_EXTRA_BYTES];
u8 WriteBuffer[WINBOND_FLASH_PAGE_SIZE + WINBOND_READ_WRITE_EXTRA_BYTES];
...
int main()
{
	 ...
	 */
	Status = XSpi_SetSlaveSelect(&Spi, WINBOND_SPI_SELECT);
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	 ...
	 */
	Status = SpiWinbondFlashWriteEnable(&Spi);
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	 ...
	 */
	Status = SpiWinbondFlashWaitForFlashNotBusy();
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	 ...
	 */
	Status = SpiWinbondFlashWriteStatus(&Spi, WINBOND_DISABLE_PROTECTION_ALL);
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	 ...
	 * operations.
	 */
	Address = WINBOND_FLASH_TEST_ADDRESS;
	 ...
	 */
	Status = SpiWinbondFlashSectorErase(&Spi, Address);
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	 ...
	 */
	Status = SpiWinbondFlashWrite(&Spi, Address, WINBOND_FLASH_PAGE_SIZE);
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	 ...
	 */
	for(Index = 0; Index < WINBOND_FLASH_PAGE_SIZE +
	           WINBOND_READ_WRITE_EXTRA_BYTES; Index++) {
	 ...
	 */
	Status = SpiWinbondFlashRead(&Spi, Address, WINBOND_FLASH_PAGE_SIZE);
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
	 ...
	 */
	for(Index = 0; Index < WINBOND_FLASH_PAGE_SIZE; Index++) {
		if(ReadBuffer[Index + WINBOND_READ_WRITE_EXTRA_BYTES] !=
					(u8)(Index + WINBOND_FLASH_TEST_BYTE)) {
			return XST_FAILURE;
		}
	}
...
int SpiWinbondFlashWriteEnable(XSpi *SpiPtr)
{
	 ...
	 */
	WriteBuffer[BYTE1] = WINBOND_COMMAND_WRITE_ENABLE;
...
int SpiWinbondFlashWrite(XSpi *SpiPtr, u32 Addr, u32 ByteCount)
{
	 ...
	 * Prepare the WriteBuffer.
	 */
	WriteBuffer[BYTE1] = WINBOND_COMMAND_PAGEPROGRAM_WRITE;
	 ...
	 * device.
	 */
	for(Index = 4; Index < ByteCount + WINBOND_READ_WRITE_EXTRA_BYTES;
						Index++) {
		WriteBuffer[Index] = (u8)((Index - 4) + WINBOND_FLASH_TEST_BYTE);
	}
	...
	TransferInProgress = TRUE;
	Status = XSpi_Transfer(SpiPtr, WriteBuffer, NULL,
				(ByteCount + WINBOND_READ_WRITE_EXTRA_BYTES));
	if(Status != XST_SUCCESS) {
		return XST_FAILURE;
	}
...
int SpiWinbondFlashRead(XSpi *SpiPtr, u32 Addr, u32 ByteCount)
{
	 ...
	 * Prepare the WriteBuffer.
	 */
	WriteBuffer[BYTE1] = WINBOND_COMMAND_RANDOM_READ;
	WriteBuffer[BYTE2] = (u8) (Addr >> 16);
	WriteBuffer[BYTE3] = (u8) (Addr >> 8);
	WriteBuffer[BYTE4] = (u8) Addr;
...
int SpiWinbondFlashBulkErase(XSpi *SpiPtr)
{
	 ...
	 * Prepare the WriteBuffer.
	 */
	WriteBuffer[BYTE1] = WINBOND_COMMAND_BULK_ERASE;
...
int SpiWinbondFlashSectorErase(XSpi *SpiPtr, u32 Addr)
{
	 ...
	 * Prepare the WriteBuffer.
	 */
	WriteBuffer[BYTE1] = WINBOND_COMMAND_SECTOR_ERASE;
	WriteBuffer[BYTE2] = (u8) (Addr >> 16);
	WriteBuffer[BYTE3] = (u8) (Addr >> 8);
	WriteBuffer[BYTE4] = (u8) (Addr);
...
int SpiWinbondFlashGetStatus(XSpi *SpiPtr)
{
	 ...
	 */
	WriteBuffer[BYTE1] = WINBOND_COMMAND_STATUSREG_READ;
...
int SpiWinbondFlashWriteStatus(XSpi *SpiPtr, u8 StatusRegister)
{
	 ...
	 * Prepare the Write Buffer.
	 */
	WriteBuffer[BYTE1] = WINBOND_COMMAND_STATUSREG_WRITE;
...
*
******************************************************************************/
int SpiWinbondFlashWaitForFlashNotBusy(void)
{
	int Status;
	u8 StatusReg;
	...
	while(1) {
		 ...
		 */
		Status = SpiWinbondFlashGetStatus(&Spi);
		if(Status != XST_SUCCESS) {
			return XST_FAILURE;
		}
